...
type ILoaffinityClient interface {
	...
	GetAddressUTXO(param interface{}) (*jsonrpc.RPCResponse, error)
}
...
func (l *LoaffinityClient) GetAddressUTXO(param interface{}) (*jsonrpc.RPCResponse, error) {
	response, err := l.rpcClient.Call("getaddressutxos", param)
	if err != nil {
		l.log.Debug("RPC call failed: %v", err.Error())
		return nil, err
	}
